...
app.post('/api/clear-history', async (req, res) => {
  ...
  if (!teacherId) {
    return res.status(400).json({ error: "Missing teacherId payload parameter" });
  }
...
app.post('/api/delete-student', async (req, res) => {
  ...
  if (!studentId) {
    return res.status(400).json({ error: "Missing studentId payload parameter" });
  }
...
app.post('/api/delete-scan', async (req, res) => {
  ...
  if (!scanId) {
    return res.status(400).json({ error: "Missing scanId payload parameter" });
  }
...
app.post('/api/scans', async (req, res) => {
  ...
  if (!fileName || !students || !Array.isArray(students) || !teacherId) {
    return res.status(400).json({ error: "Missing required parameters: fileName, students, teacherId" });
  }
...
app.get('/api/reports/:studentId', (req, res) => {
  ...
  // Alternate mock retrieval
  res.status(404).json({ error: "Detailed analytics report not found" });
});
...
app.post('/api/re-analyze', async (req, res) => {
  ...
  if (!report) {
    return res.status(404).json({ error: "Report data structure not found" });
  }

  const targetFile = report.files[fileIndex];
  if (!targetFile) {
